...
class MPC(ABC):
    ...
    def terminal_controller(self):
        """`terminal_controller(x)` - evaluates the terminal controller for given state x
        Default: :code:'None'.
        """
    ...
    def stabilizing_feedback_controller(self):
        """`stabilizing_feedback_controller(x, v)` - evaluates the stabilizing feedback controller for given state x and inputs v, returns feedback u
        Default: :code:'None'.
        """
...
class MPCQuadraticCostLxLu(MPC):
    ...
    def feasible(self,X,U, verbose=False, only_states=True, robust=False, eps=0.0001):
        ...
        if verbose and not res:
            ...
            print("\tin terminal constraint: ", self.in_terminal_constraints(X[-1,:], verbose=True  , robust=robust))
        return res
    ...
    def stabilizing_feedback_controller_clipped_inputs(self, x, v):
        return np.clip(self.stabilizing_feedback_controller(x,v), self.__umin, self.__umax)
...
def import_mpc(file="latest", mpcclass=MPCQuadraticCostLxLu):
    ...
    p = DATASETS_DIR.joinpath(file, "parameters")
