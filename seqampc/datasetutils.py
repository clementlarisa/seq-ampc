...
def append_to_dataset(mpc, x0dataset, Udataset, Xdataset, computetimes, filename):
    ...
    p = DATASETS_DIR.joinpath(filename, "data")
...
def export_dataset(mpc, x0dataset, Udataset, Xdataset, computetimes, filename, barefilename=False):    
    ...
    p = DATASETS_DIR.joinpath(datasetname, "data")
    p.mkdir(parents=True,exist_ok=True)
    ...
    mpc.savetxt(DATASETS_DIR.joinpath(datasetname, "parameters"))

    print("Exported to directory:\n\t",  DATASETS_DIR.joinpath(datasetname).absolute(),"\n")

    target = datasetname
    link_name=DATASETS_DIR.joinpath("latest")
    try:
...
def import_dataset(mpc, file="latest", return_aux=False):
    ...
    """
    root = DATASETS_DIR.joinpath(file)
