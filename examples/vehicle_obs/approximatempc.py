...
def _load_augmented_inputs(mpc, dataset):
    """Load dataset and augment state with obstacle parameters."""
    ...
    if P_obstacles.shape[1] < 5:
        raise ValueError(
            f"Expected P_obstacles to have at least 5 columns, got shape {P_obstacles.shape}"
        )

    # Drop last column (N_active)
...
def find_approximate_mpc(neural_type="MLP", dataset="latest", rnn_units=32, dense_units = (200, 400, 600, 600, 400, 200,), retrain=False, retrain_model_name="latest"):
    ...
    X_aug, U = _load_augmented_inputs(mpc, dataset)
