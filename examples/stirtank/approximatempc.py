...
def find_approximate_mpc(neural_type="MLP", dataset="latest"):
    # import latest dataset :-D
    mpc = import_mpc(dataset, MPCQuadraticCostLxLu)
    X0, U, X, _ = import_dataset(mpc, dataset)

    # define architectures to be tested
    architectures_mlp = np.array([
    ...
    architectures_lstm = np.array([
        [mpc.nx, 32, mpc.nu * mpc.N],
    ])
    hyperparameters = [{"learning_rate": 0.01, "patience": 10, "max_epochs": 10, "batch_size": 10000}, # TODO change form 10 back to 1000
    ...
    model = architecture_search(mpc, X0, U, NeuralType.MLP if neural_type == "MLP" else NeuralType.LSTM, hyperparameters=hyperparameters,
                                architectures=architectures_mlp if neural_type == "MLP" else architectures_lstm)
    return None
