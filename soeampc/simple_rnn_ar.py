...
@keras_export("keras.layers.SimpleRNN_AR_Cell")
class SimpleRNN_AR_Cell(Layer, DropoutRNNCell):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.state_size = initial_hidden_size
    ...
    def call(self, sequence, states, training=False):
        ...
        # Calculate new hidden state based on previous one
        prev_state = states[0] if isinstance(states, (list, tuple)) else states
        new_state = ops.matmul(prev_state, self.recurrent_kernel) + self.bias
        if self.activation is not None:
            new_state = self.activation(new_state)
        
        # Calculate new output based on new hidden state
        output = ops.matmul(new_state, self.output_kernel) + self.output_bias
        if self.activation is not None:
            output = self.activation(output)
        
        new_state = [new_state] if isinstance(states, (list, tuple)) else new_state
...
@keras_export("keras.layers.SimpleRNN_AR")
class SimpleRNN_AR(RNN):
...
@tf.keras.utils.register_keras_serializable(package="soeampc")
class RNN_AR_RolloutModel(keras.Model):
    def __init__(self, T, rnn_units=32, dense_units=(50,), out_dim=3, **kwargs):
        super().__init__(**kwargs)

        self.T = int(T)
        self.out_dim = int(out_dim)

        # Store these so they can go into get_config
        self.rnn_units = rnn_units
        self.dense_units = tuple(int(x) for x in dense_units)

        self.x_norm = layers.Normalization(input_shape=[int(T), ], axis=None)
        self.pre_dense = [layers.Dense(du, activation="tanh") for du in self.dense_units]

        self.encoder_rnn = SimpleRNN_AR_Cell(dense_units[-1], out_dim)
        # self.head = layers.Dense(self.out_dim, activation="linear")

    def adapt_normalizer(self, traindata_seq):
        self.x_norm.adapt(traindata_seq)

    def call(self, inputs):
        x = self.x_norm(inputs)

        for d in self.pre_dense:
            x = d(x)
        ...
        for _ in range(self.T):
            out, h = self.encoder_rnn(input, h)
            # y = self.head(out)
            outputs.append(out)
    ...
    def get_config(self):
        ...
        config.update({
            ...
            "rnn_units": int(self.rnn_units),              # <-- FIX
            "dense_units": list(self.dense_units),         # ok
        })
        return config
    ...
    @classmethod
    def from_config(cls, config):
        # Keras may pass extra keys; safest to pop what you consume.
        return cls(**config)
