...
class RNNRolloutModel(keras.Model):
    def __init__(self, T, rnn_units=32, dense_units=(50,), out_dim=3, **kwargs):
        ...
        self.encoder_rnn = SimpleRNNCell(rnn_units)
        # self.decoder_cell = layers.SimpleRNNCell(self.rnn_units[-1])
    ...
    def call(self, inputs):
        ...
        h = tf.zeros((self.rnn_units), dtype=inputs.dtype)
        # batch_size = tf.shape(inputs)[0]
        # dec_in = tf.zeros((batch_size, 1), dtype=inputs.dtype)
        outputs = []
        # state = [h]
        for _ in range(self.T):
            # print(f"-------------------RNNRolloutModel--------------")
...
class SimpleRNNCell(Layer, DropoutRNNCell):
    ...
    def call(self, sequence, states, training=False):
        ...
        # print(f"PrevOutput: {prev_output.shape}")
